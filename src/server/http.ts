...
const DEFS: Array<[string, string, (ctx: Ctx) => Promise<unknown> | unknown]> = [
  ...
  ["GET", "/admin/email/list", ({ token, query }) => core.adminListEmails(token, query.get("folder") ?? "draft")],
  ["POST", "/admin/email/draft", ({ token, body }) => core.adminSaveDraft(token, body)],
  ["POST", "/admin/email/send", ({ token, body }) => core.adminSendEmail(token, body)],
  ["POST", "/admin/email/trash", ({ token, body }) => core.adminTrashEmail(token, body.id)],
  ["POST", "/admin/email/restore", ({ token, body }) => core.adminRestoreEmail(token, body.id)],
  ["POST", "/admin/email/delete", ({ token, body }) => core.adminDeleteEmail(token, body.id)],
